...
- **灵活的客户端配置**: 支持自定义请求头 (`headers`)、提示词前缀 (`special_prefix`)、停止序列 (`stop`) 和最大令牌数 (`max_tokens`)。
...
- **数据清洗**: 自动移除特定模型的 `<think>` 思考标签，通过静态编译的正则引擎高效处理。
...
docker run -d \
  ...
  -v $(pwd)/logs:/app/logs \
  --restart always \
  ghcr.io/diannaojiang/openai-api:b245
```
